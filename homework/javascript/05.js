...
// TODO: 랜덤 할일 조회
async function randomTodo(random) {
...
async function randomQuote() {
  const res = await axios.get(`${BASE_URL}/quote/random`);
  const data = res["data"];
  console.log(data);
}
// randomQuote();
...
async function recipesTag() {
  const res = await axios.get(`${BASE_URL}/recipes/tag/beef`);
  const data = res["data"];
  console.log(data);
}
// recipesTag();
...
async function cartUserId(num) {
  const res = await axios.get(`${BASE_URL}/carts/user/${num}`);
  const data = res["data"];
  console.log(data);
}
cartUserId(6);
