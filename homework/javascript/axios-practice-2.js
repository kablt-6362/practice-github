...
async function productRating(id) {
  const res = await axios.get(`${BASE_URL}/products/${id}`);
  const reviews = res["data"]["reviews"];
  //reviews 는 id10번 상품의 리뷰가 담겨있는 배열. {리뷰1,리뷰2,리뷰3}
  //나는 reviews 배열의 인덱스의 값, 객체에 접근 -> 그객체의 key값이 rating인 것의 value가 필요해
  //reviews 는 배열이고, forEach를 통해 안의 인덱스의 수만큼 반복될거고 반복중인 해당 객체의 key가 rating에 접근해야됨
  //ele는 반복중 현재 인덱스의 값
  let count = 0;
  // ele는 현재값이다.ele["rating"]은 reviews의 현재 값의 rating
  reviews.forEach((ele) => {
    const ratNum = ele["rating"];
    count = count + ratNum;
  });
...
async function postList() {
  ...
  const data = res["data"]["posts"]; //지금 있는건 posts의 객체들
